...
void CSplitterBar::SetPanes(CWnd *pwndLeftPane,CWnd *pwndRightPane)
{
	...
	if(m_bHorizontal)
	{
		...
		// Modified by Tihamer Levendovszky 2002 July
		CRect rectRightPane;
		pwndRightPane->GetWindowRect(rectRightPane);
		GetParent()->ScreenToClient(rectRightPane);
		int nDist=rectRightPane.top-rectBar.bottom;
...
#include "resource.h"
void CSplitterBar::ChangeHelpControls(CRect ctrlRect)
{
	...
	if (pHelpTitleCtrl->GetSafeHwnd())
	{
		...
		CWnd* pHelpTextCtrl=GetParent()->GetDlgItem(IDC_HELP_TEXT);
		if(pHelpTextCtrl->GetSafeHwnd())
		{
			pHelpTextCtrl->SetWindowPos(NULL,
										ctrlRect.left+1,
										ctrlRect.top+INSP_HELP_TITLE_HEIGHT+2,
										ctrlRect.Width()-2,
										ctrlRect.Height()-INSP_HELP_TITLE_HEIGHT-3,
										SWP_NOZORDER);
		}
	}
