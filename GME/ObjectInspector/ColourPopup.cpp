...
IMPLEMENT_DYNAMIC(CColourPopup, CDialog)
...
CColourPopup::CColourPopup(CWnd* pParent /*=NULL*/)
	: CDialog(CColourPopup::IDD, pParent)
{
	m_bFirstMouseMsgReceived = false;
}
...
void CColourPopup::SetParameters(CRect rectBound, COLORREF crColour, bool rightSideClick,
								 LPCTSTR szDefaultText, LPCTSTR szCustomText)
{
	m_RectBound      = rectBound;
    m_crColour       = m_crInitialColour = crColour;
	m_bRightSideClick= rightSideClick;
...
BOOL CColourPopup::OnInitDialog() 
{
	CDialog::OnInitDialog();

    Initialize();

    ASSERT(GetParent() && ::IsWindow(GetParent()->GetSafeHwnd()));
	...
	int nXOffset = m_RectBound.right - rectWnd.right;
	SetWindowPos(NULL, rectWnd.left + nXOffset, m_RectBound.top, 0, 0, SWP_NOSIZE | SWP_NOZORDER | SWP_SHOWWINDOW);
	SetWindowSize();

	if (!m_bRightSideClick)
...
void CColourPopup::EndDialog(int nResult)
{
	ReleaseCapture();
