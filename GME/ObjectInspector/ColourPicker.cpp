...
void CColourPicker::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
   ...
   // @T. L. pDC->DrawEdge(rect, EDGE_SUNKEN, BF_RECT);
...
void CColourPicker::SetWindowSize()
{
   ...
   // int nArrowWidth = 14;//max(::GetSystemMetrics(SM_CXHTHUMB), 5*MarginSize.cx);
   // int nArrowHeight = 14;//max(::GetSystemMetrics(SM_CYVTHUMB), 5*MarginSize.cy);
	...
	// Set Arrow size @Tihamer Levendovszky
	CSize ArrowSize(rect.Height(),rect.Height());
  ...
  /*  m_ArrowRect.SetRect(rect.right - ArrowSize.cx - MarginSize.cx, 
					...
					*/
	// @Tihamer Levendovszky
	 m_ArrowRect.SetRect(rect.right - ArrowSize.cx,rect.top-1,rect.right,rect.bottom-1 );
}
